...
try:
    ...
    # 2. Row count
    cur.execute("SELECT COUNT(*) FROM dbo.PROPERTY_0")
    print(f"\nPROPERTY_0 row count: {cur.fetchone()[0]}")

    # 3. Sample FLAG columns to understand statuses
    cur.execute("""
    ...
    # 5. PROPERTY_0 sample row
    print("\n=== Sample PROPERTY_0 row (first active) ===")
    cur.execute("SELECT TOP 1 * FROM dbo.PROPERTY_0 WHERE FLAG_ACTIVE = 1 ORDER BY PROPERTY_NAME")
